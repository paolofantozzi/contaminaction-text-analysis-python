...
def load_docs():
    ...
    edges = defaultdict(lambda: defaultdict(lambda: {'weight': 0}))
    for work in tqdm(file_paths[:1000]):
        ...
        domain = doc['domain']
        for url, _, _ in doc['links']:
            target_domain = urlparse(url).netloc
            edges[domain][target_domain]['weight'] += 1
        docs.append(doc)
...
def merge_results(content_results, pars_results, title_results, domain_scores):
    all_doc_ids = set().union(content_results.keys(), pars_results.keys(), title_results.keys())
    results = []
    for doc_id in all_doc_ids:
        the_doc = (content_results.get(doc_id, None) or pars_results.get(None, doc_id) or title_results[doc_id]).doc
        ...
        tot_score *= domain_scores[the_doc['domain']]
        pos = content_result.pos
        results.append(Result(the_doc, pos, tot_score))
    return results

def load_clusters():
    clusters_file_path = Path('works') / 'clusters.json'
    with open(clusters_file_path, 'r') as clusters_file:
        clusters = json.load(clusters_file)
    return clusters

def save_query(query):
    history_path = Path('works') / 'query_history.txt'
    with open(history_path, 'a') as history_file:
        history_file.write(f'{query}\n')

def search_history(query_start, how_many=None):
    history_path = Path('works') / 'query_history.txt'
    counter = Counter()
    with open(history_path, 'r') as history_file:
        for query in history_file:
            query = query.strip()
...
if __name__ == '__main__':
    ...
    cluster_weights = {domain: 0.1 for domain in clusters}
    min_cluster_idx = min(clusters.values())
    max_cluster_idx = max(clusters.values())
    topic_idx = int(input(f'A quale topic sei interessato? [{min_cluster_idx}-{max_cluster_idx}]\n'))
    cluster_weights = cluster_weights | {domain: 1 for domain, topic in clusters.items() if topic == topic_idx}
    domain_scores = nx.pagerank(graph, personalization=cluster_weights)
    ...
    if set(query) & {'!', '&', '|'}:
        ...
        title_results = {}
        pars_results = {}
        for hist_query, hist_count in search_history(correct_query, 10):
            print(f'{hist_query} -> {hist_count}')
    else:
        correct_query = corrector.correct(query)
        if query != correct_query:
            print('Risultati per:')
            print(correct_query)
        save_query(correct_query)
    ...
    results = merge_results(content_results, pars_results, title_results, domain_scores)
    print('Risultati:')
